...
## Сборка одной командой в Docker

На Linux с установленным Docker Engine можно поднять полностью настроенный
Alpine 3.20.6 build-root и собрать Telegram одной командой. Dockerfile
устанавливает только пакеты из v3.20, Rust 1.96.1 и все инструменты,
перечисленные ниже; исходники и сабмодули клонируются по SHA из
`versions.env`.

Сначала получи API ID и API hash для Telegram Desktop и задай их в окружении
сборки. В репозиторий эти значения записывать нельзя:

```sh
export TDESKTOP_API_ID=123456
export TDESKTOP_API_HASH='0123456789abcdef0123456789abcdef'
./packaging/alpine/docker-build.sh
```

Скрипт автоматически:

1. собирает образ `alpine:3.20.6` под `linux/amd64`;
2. клонирует основной репозиторий, TDLib, tlottie и tg_owt по
   зафиксированным коммитам;
3. собирает TDLib, TDLib E2E, tlottie с `c-api`, tg_owt и Telegram Desktop;
4. экспортирует результат в `out/alpine/`.

Основной результат находится в `out/alpine/Telegram`, рядом лежат
desktop-файлы и `build-info.txt`. Исходники и build-зависимости в результат
не копируются. Для сохранения полного staging-root добавь
`EXPORT_STAGING=1`; тогда появится `out/alpine/telegram-root.tar.gz`.

По умолчанию используется `linux/amd64`. На ARM64-хосте Docker сможет
запустить эту сборку через эмуляцию, но она будет значительно медленнее:

```sh
DOCKER_PLATFORM=linux/amd64 JOBS=8 ./packaging/alpine/docker-build.sh
```

Требуется интернет для загрузки образа, APK-пакетов, исходников и Cargo
зависимостей. Для офлайн-сборки их нужно заранее закешировать или заменить
источники локальным зеркалом. Скрипт собирает бинарник и staging-root; APK
из него пока не создаётся автоматически.
